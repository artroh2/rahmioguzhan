...
const StarfieldCanvas = () => {
  ...
  const initNebulae = useCallback((w: number, h: number) => {
    nebulaeRef.current = [
      { x: w * 0.2, y: h * 0.3, radius: Math.max(w, h) * 0.25, color: '59, 15, 110', rotationSpeed: 0.0003, breatheSpeed: 0.0008, breathePhase: 0 },
      { x: w * 0.75, y: h * 0.6, radius: Math.max(w, h) * 0.3, color: '10, 22, 40', rotationSpeed: -0.0002, breatheSpeed: 0.0006, breathePhase: 1.5 },
      { x: w * 0.5, y: h * 0.8, radius: Math.max(w, h) * 0.2, color: '107, 15, 74', rotationSpeed: 0.0004, breatheSpeed: 0.001, breathePhase: 3 },
    ];
  }, []);
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      initNebulae(window.innerWidth, document.documentElement.scrollHeight);
    };
